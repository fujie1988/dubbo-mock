...
echo "-----------------------------"
    ...
    exec $_RUNJAVA $JAVA_OPTS -classpath "$CLASSPATH" \
        -Droot.path="$SERVER_ROOT_PATH" \
        com.lianjia.house.api.SeApiMain "$@" \
        >> "$STDOUT_LOG" 2>&1
        
    echo "$_RUNJAVA $JAVA_OPTS -classpath "$CLASSPATH" -Droot.path="$SERVER_ROOT_PATH" com.lianjia.retrieval.test.dubbo.DubboProxyMain $@"
